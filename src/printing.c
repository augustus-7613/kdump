...
void print_krb5_ticket(const int level, const krb5_ticket* tkt)
{
    ...
    if (args.verbose) print_kv_int(level+1, "Magic", tkt->magic);

    print_indent(level+1); printf("Server: \n");
    if (args.verbose) print_kv_int(level+2, "Magic", tkt->server->magic);
    if (args.verbose) print_kv_int(level+2, "Length", tkt->server->length);
    print_kv_int(level+2, "Type", tkt->server->type);

    print_indent(level+2); printf("Realm: \n");
    if (args.verbose) print_kv_int(level+3, "Magic", tkt->server->realm.magic);
    if (args.verbose) print_kv_int(level+3, "Length", tkt->server->realm.length);
    ...
    for (i = 0; i < tkt->server->length; i++) {
        ...
        if (args.verbose) print_kv_int(level+3, "Magic", tkt->server->data[i].magic);
        if (args.verbose) print_kv_int(level+3, "Length", tkt->server->data[i].length);
        print_kv_str(level+3, "Data", tkt->server->data[i].data, tkt->server->data[i].length);
    }

    print_indent(level+1); printf("Enc_part: \n");
    if (args.verbose) print_kv_int(level+2, "Magic", tkt->enc_part.magic);
    ...
    if (args.verbose) print_kv_int(level+3, "Magic", tkt->enc_part.ciphertext.magic);
    if (args.verbose) print_kv_int(level+3, "Length", tkt->enc_part.ciphertext.length);
    ...
    if (args.verbose) print_kv_int(level+2, "Magic", tkt->enc_part2->magic);
    print_kv_int(level+2, "Flags", tkt->enc_part2->flags);

    print_indent(level+2); printf("Client: \n");
    if (args.verbose) print_kv_int(level+3, "Magic", tkt->enc_part2->client->magic);
    if (args.verbose) print_kv_int(level+3, "Length", tkt->enc_part2->client->length);
    ...
    for (i = 0; i < tkt->enc_part2->client->length; i++) {
        ...
        if (args.verbose) print_kv_int(level+4, "Magic", tkt->enc_part2->client->data[i].magic);
        if (args.verbose) print_kv_int(level+4, "Length", tkt->enc_part2->client->data[i].length);
    ...
    if (args.verbose) print_kv_int(level+4, "Magic", tkt->enc_part2->client->realm.magic);
    if (args.verbose) print_kv_int(level+4, "Length", tkt->enc_part2->client->realm.length);
    ...
    if (tkt->enc_part2->authorization_data != NULL)
    {
        ...
        for (i = 0, authdata = tkt->enc_part2->authorization_data; *authdata != NULL; authdata++, i++) {
            ...
            if (args.verbose) print_kv_int(level+3, "Magic", (*authdata)->magic);
            if (args.verbose) print_kv_int(level+3, "Length", (*authdata)->length);
    ...
    if (tkt->enc_part2->caddrs != NULL)
    {
        ...
        for (i = 0, addresses = tkt->enc_part2->caddrs; *addresses != NULL; addresses++, i++) {
            ...
            if (args.verbose) print_kv_int(level+3, "Magic", (*addresses)->magic);
            if (args.verbose) print_kv_int(level+3, "Length", (*addresses)->length);
    ...
    if (args.verbose) print_kv_int(level+3, "Magic", tkt->enc_part2->session->magic);
    if (args.verbose) print_kv_int(level+3, "Length", tkt->enc_part2->session->length);
    ...
    if (args.verbose) print_kv_int(level+3, "Magic", tkt->enc_part2->transited.magic);
...
void print_krb5_cred(const krb5_context ctx, const krb5_creds* creds, const krb5_ticket* tkt)
{
    ...
    if (args.verbose) print_kv_int(0, "Magic", creds->magic);
    ...
    for (i = 0, authdata = creds->authdata; *authdata != NULL; authdata++, i++)
    {
        ...
        if (args.verbose) print_kv_int(2, "Magic", creds->authdata[i]->magic);
        print_kv_int(1, "AdType", creds->authdata[i]->ad_type);
        if (args.verbose) print_kv_int(1, "Length", creds->authdata[i]->length);
    ...
    for (i = 0, addresses = creds->addresses; *addresses != NULL; i++, addresses++)
    {
        ...
        if (args.verbose) print_kv_int(1, "Magic", creds->addresses[i]->magic);
        print_kv_int(1, "AddrType", creds->addresses[i]->addrtype);
        if (args.verbose) print_kv_int(1, "length", creds->addresses[i]->length);
        print_kv_str(1, "Contents", creds->addresses[i]->contents, creds->addresses[0]->length);
    }

    print_indent(0); printf("\n[Client]\n");
    if (args.verbose) print_kv_int(1, "Magic", creds->client->magic);
    if (args.verbose) print_kv_int(1, "Length", creds->client->length);
    print_kv_int(1, "Type", creds->client->type);

    print_indent(1); printf("Realm:\n");
    if (args.verbose) print_kv_int(2, "Magic", creds->client->realm.magic);
    if (args.verbose) print_kv_int(2, "Length", creds->client->realm.length);
    print_kv_str(2, "Data", creds->client->realm.data, creds->client->realm.length);

    print_indent(1); printf("Data: \n");
    if (args.verbose) print_kv_int(2, "Magic", creds->client->data->magic);
    if (args.verbose) print_kv_int(2, "Length", creds->client->data->length);
    print_kv_str(2, "Data", creds->client->data->data, creds->client->data->length);

    print_indent(0); printf("\n[Server]\n");
    if (args.verbose) print_kv_int(1, "Magic", creds->server->magic);
    if (args.verbose) print_kv_int(1, "Length", creds->server->length);
    print_kv_int(1, "Type", creds->server->type);

    print_indent(1); printf("Realm: \n");
    if (args.verbose) print_kv_int(2, "Magic", creds->server->realm.magic);
    if (args.verbose) print_kv_int(2, "Length", creds->server->realm.length);
    ...
    for (i=0; i<creds->server->length; i++)
    {
        ...
        if (args.verbose) print_kv_int(2, "Magic", creds->server->data[i].magic);
        if (args.verbose) print_kv_int(2, "Length", creds->server->data[i].length);
    ...
    print_indent(0); printf("\n[KeyBlock]\n");
    if (args.verbose) print_kv_int(1, "Magic", creds->keyblock.magic);
    if (args.verbose) print_kv_int(1, "Length", creds->keyblock.length);
    print_kv_int(1, "EncType", creds->keyblock.enctype);
    print_kv_bytes(1, "Contents", creds->keyblock.contents, creds->keyblock.length);

    print_indent(0); printf("\n[SecondTicket]\n");
    if (args.verbose) print_kv_int(1, "Magic", creds->second_ticket.magic);
    if (args.verbose) print_kv_int(1, "Length", creds->second_ticket.length);
